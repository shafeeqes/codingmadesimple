...
rod of length 6 (i.e. the second part of the first cut), the third is done to a rod of length 4 and 
the last cut is to a rod of length 3. The total cost is 7 + 6 + 4 + 3 = 20.
...
public:
    ...
    int minCost(int n, vector<int>& A) {
        ...
        return solve(0, s-1, 0, n);
    }
    
    int solve(int i, int j, int l, int r){
        
        if( i < 0 or j >= cuts.size() or i > j)
            return 0;
        ...
        for(int k = i; k <= j; k++) {
            ans = min(ans, (r - l) + solve(i, k - 1, l, cuts[k]) + solve(k + 1, j, cuts[k], r));
        }
